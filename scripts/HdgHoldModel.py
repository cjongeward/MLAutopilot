...
class HdgHoldNN(nn.Module):
    InputNames = ["hdg", "dhdg", "bank"]
    OutputNames = ["aileronPos"]
    DatasetFilename = "hdgHoleData.csv"
    ModelFileName = "hdgHoldModel.pth"

    def __init__(self):
        super(HdgHoldNN, self).__init__()
        layer1 = nn.Linear(len(HdgHoldNN.InputNames), 64)  # Input to hidden layer
        layer2 = nn.Linear(64, 64)  # Hidden layer to output
        layer3 = nn.Linear(64, 64)  # Hidden layer to output
        layer4 = nn.Linear(64 ,len(HdgHoldNN.OutputNames))  # Hidden layer to output
    ...
    def load_datasets(self):
        self.dataset = getDataset("hdgHoldData.csv", HdgHoldNN.InputNames, HdgHoldNN.OutputNames)
